...
public class JavaStackFrameSourceDisplayAdapter implements ISourceDisplay {
	...
	public void displaySource(Object element, IWorkbenchPage page, boolean forceSourceLookup) {
		...
		try {
			...
			if (sourceRes.getEditorInput() instanceof IClassFileEditorInput) {
				if (openClassFileAndHighlightStackFrame(page, sourceRes, jdiFrame)) {
			...
			if (LambdaUtils.isLambdaFrame(jdiFrame)) {
				if (handleLambda(page, jdiFrame, sourceRes)) {
					return;
				}
			}
	...
	private boolean handleLambda(IWorkbenchPage page, JDIStackFrame jdiFrame, SourceLookupResult sourceRes) throws CoreException, BadLocationException {
		...
		IEditorPart part = openInEditor(page, sourceRes);
	...
	private boolean openClassFileAndHighlightStackFrame(IWorkbenchPage page, SourceLookupResult lookupResult, JDIStackFrame jdiFrame) throws JavaModelException, PartInitException, DebugException {
		IEditorPart part = openInEditor(page, lookupResult);
		// check if it is a ClassFileEditor with no source attached
		if (part instanceof ClassFileEditor editor && editor.getDocumentProvider().getDocument(editor.getEditorInput()).getLength() == 0) {
			...
			editor.highlightInstruction(methodName, jdiFrame.getSignature(), jdiFrame.getCodeIndex());
	...
	private IEditorPart openInEditor(IWorkbenchPage page, SourceLookupResult lookupResult) throws PartInitException, JavaModelException {
		IJavaElement element = JavaUI.getEditorInputJavaElement(lookupResult.getEditorInput());
		if (element == null) {
			return null;
		}
		// To allow reusing editors, try to display the lookup result using the SourceLookupFacility first
		SourceLookupFacility.getDefault().display(lookupResult, page);
		// Then, openInEditor should find opened element
		// This should typically be the active part but it may be null if it couldn't display any corresponding editor for some reason
